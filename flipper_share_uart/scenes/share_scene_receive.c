...
static int32_t file_read_worker_thread(void* context) {
    ...
    while(is_running) {
        ...
        if(finished) {
            // Reception is over (Success or Hash failed). The full-duplex UART has
            // no field to stop; the transport is torn down in on_exit. Just keep
            // RX running harmlessly until the scene exits.
...
static void progress_view_draw_callback(Canvas* canvas, void* context) {
    ...
    elements_multiline_text_aligned(canvas, 64, SCENE_HEADER_POSITION_Y, AlignCenter, AlignTop, "Receiving via " FSH_TRANSPORT_NAME "...");
...
void share_scene_receive_on_enter(void* context) {
    ...
    // UART transport, so both threads see a valid mutex from their first tick.
    fsh_lock_ensure();
    ...
    dialog_ex_set_text(app->dialog_show_file, "13-14 X, GND-GND", 64, 32, AlignCenter, AlignCenter);
    ...
    // Acquire USART first. On failure the port is busy — show it and leave the
    // worker/timer unstarted; the user backs out with Back.
    if(!uart_transport_init()) {
        dialog_ex_set_text(
            app->dialog_show_file, "UART port busy", 64, 32, AlignCenter, AlignCenter);
        return;
    }
...
static void update_timer_callback(void* context) {
    ...
    if(complete) {
        ...
        snprintf(progress_text, sizeof(progress_text), "13-14 X, GND-GND\nWaiting for announce...");
...
void share_scene_receive_on_exit(void* context) {
    ...
    // Worker thread is joined in on_event and the UART transport is stopped above,
