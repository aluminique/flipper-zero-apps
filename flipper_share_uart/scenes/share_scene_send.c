...
#define FSH_IDLE_OPERATION 5 //ms (paces fsh_idle; the blocking UART send is the real pacing)
...
void share_scene_send_on_enter(void* context) {
    ...
    // Acquire USART before doing any work. On failure the port is held by
    // someone else — show the reason and leave the worker/timer unstarted; the
    // user backs out with Back.
...
static void update_timer_callback(void* context) {
    ...
    if(state->reading_complete) {
        ...
        // failed) — keep the "13-14 X, GND-GND" hint from on_enter.
        if(fsize == 0) return;
