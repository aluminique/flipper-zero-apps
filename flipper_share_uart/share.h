...
#define FSH_ETA_MAX_SEC (99u*3600u + 59u*60u + 59u)          // clamp ETA display (avoid uint32 overflow / garbage)
...
typedef struct {
    ...
    uint32_t (*cb_read_block)(uint32_t block_number, uint8_t out[FSH_DATA_LENGTH]);        // Reader callback from real storage
...
typedef struct {
    ...
    void (*r_write_block)(uint32_t block_number, const uint8_t in[FSH_DATA_LENGTH], uint32_t valid_len);
} fsh_init_params_t;
